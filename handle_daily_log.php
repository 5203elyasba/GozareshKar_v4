...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    header("location: new_log.php"); // Redirect back if not a POST request
    exit;
}
...
$log_date_jalali = $_POST['log_date'] ?? '';
...
if (empty($log_date_jalali)) {
    ...
    header("location: new_log.php?error=تاریخ مشخص نشده است.");
    exit;
}
...
if (!$gregorian_date_obj) {
    header("location: new_log.php?error=فرمت تاریخ نامعتبر است.");
    exit;
}
...
try {
    ...
    // 1. Delete existing records for this day and user
    ...
    // 2. Insert new records based on day type
    if ($day_type === 'work' || $day_type === 'friday_work') {
        // Insert day property first
    ...
    } else { // For 'leave' or 'official_holiday'
        $sql_prop = "INSERT INTO day_properties (user_id, log_date, day_type, status) VALUES (:user_id, :log_date, :day_type, 'approved')";
        $stmt_prop = $pdo->prepare($sql_prop);
        $stmt_prop->execute([
            ':user_id' => $user_id,
    ...
    header("location: new_log.php?date=" . urlencode($log_date_jalali) . "&success=گزارش با موفقیت ثبت شد.");

} catch (Exception $e) {
    $pdo->rollBack();
    error_log('Handle Daily Log Error: ' . $e->getMessage());
    header("location: new_log.php?date=" . urlencode($log_date_jalali) . "&error=خطای دیتابیس رخ داد. لطفا دوباره تلاش کنید.");
}
?>